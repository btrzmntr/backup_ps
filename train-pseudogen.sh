...
echo $PYTHON_FILE 1>&2
echo "tokenizing python ... " 1>&2
python $BASE_DIR/scripts/tokenize-py.py < $PYTHON_FILE > $PYTHON_FILE.pytok
echo "tokenizing english ... " 1>&2
python $BASE_DIR/scripts/tokenize-en.py < $ENG_FILE > $ENG_FILE.entok
echo "parsing python ... " 1>&2
python $BASE_DIR/scripts/parse.py < $PYTHON_FILE > $PYTHON_FILE.rawtree
echo "head insertion ... " 1>&2
python $BASE_DIR/scripts/head-insertion.py < $PYTHON_FILE.rawtree > $PYTHON_FILE.headtree
echo "simplifying ... " 1>&2
python $BASE_DIR/scripts/simplify.py < $PYTHON_FILE.headtree > $PYTHON_FILE.reducedtree
#paste $PYTHON_FILE.pytok $ENG_FILE.entok $PYTHON_FILE.reducedtree > all.combined
python $BASE_DIR/scripts/filter-data.py $PYTHON_FILE.pytok $ENG_FILE.entok $PYTHON_FILE.reducedtree > all.combined
...
for d in train dev test; do
    ...
    python $BASE_DIR/scripts/extract_words.py word < $d.reducedtree > $d.reducedsurf
done

#echo "making alignment ... " 1>&2
#mkdir align
#$BASE_DIR/tools/pialign/src/lib/pialign.cc train.reducedsurf train.entok align/align.
